...
def main():
    ...
    for elem in backup_dirs:
        if (elem in ["cluster-node-backups"]):
            delete_old_backup_enteries(os.path.join(BACKUP_ROOT, 'cluster-node-backups'), BACKUPS_KEEP_DAYS,
                                       BACKUPS_MINCOUNT)
            invalid_dir = False
        if (elem in ["inventory-summary"]):
            delete_old_backup_enteries(os.path.join(BACKUP_ROOT, 'inventory-summary'), BACKUPS_KEEP_DAYS,
                                       BACKUPS_MINCOUNT)
            invalid_dir = False
    if invalid_dir:
